    ...
    function searchController($filter, $location, $rootScope, $uibModal, $timeout, requestToListener, searchService, firebaseFactory, userAuthorizationService, encryptionService) {
        ...
        vm.searchFormSubmit = function () {
            ...
            if (vm.formData.codeFormat.status == 'valid' && vm.patientIdFormat.status == 'valid') {
                ...
                userAuthorizationService.setUserData(vm.formData.formFieldsData.registrationCode, vm.formData.hospitalCode);
                ...
                if (vm.patientId.length == 12) {
                    ...
                    userAuthorizationService.setUserRAMQ(vm.formData.formFieldsData.ramq);
                    vm.createBranchName();

                } else if (vm.patientId.length == 7) {
                    vm.formData.formFieldsData.mrn = vm.patientId;
                    userAuthorizationService.setUserMRN(vm.formData.formFieldsData.mrn);
                    vm.getUserRamqByMRN();
                }
            }
        };

        vm.getUserRamqByMRN = function() {
            let parameters = {
                'mrn': vm.formData.formFieldsData.mrn,
            };
            requestToListener.sendRequestWithResponse('GetRamqByMRN', { Fields: parameters })
                .then(function (response) {
                    if (response.Result == 'SUCCESS') {
                        // Call function to validate IPAddress.
                        vm.formData.formFieldsData.ramq = response.Data.SSN;
